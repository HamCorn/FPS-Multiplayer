...
{
    float rotX;//stores rotation in the X axis (存儲 X 軸的旋轉)
    float rotY;//stores rotation in the Y axis (存儲 Y 軸的旋轉)
    public float mouseSmoothness = 100;//basically mouse Sensitivity (基本上是鼠標靈敏度)
    public float mouseRotAcceleration = 20;
    public float followSpeed = 100;
    public float maxPitch = 80;
    public float fovAim;
    public float fovNorm;
    public Transform yaw;//yaw gameObject's transform (偏航遊戲對象的變換)
